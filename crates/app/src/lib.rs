...
impl Gpu {
    ...
    fn frame_timing(
        ...
        if now.duration_since(self.perf.last_title) >= Duration::from_millis(250) {
            ...
            if let Some(lines) = landing {
                // One line per row, at the readout's pitch: the ground has
                // up to three things to say (the bitmap is 96 rows; row
                // 72 + a glyph still fits).
                for (i, line) in lines.lines().take(3).enumerate() {
                    self.text.draw(0, 60 + 6 * i, line);
                }
            }
        }
