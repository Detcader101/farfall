...
| `FARFALL_VR_HANDS=real\|synth` | `synth` if `FARFALL_VR=synth`, else `real` | Which controller source drives `Game::vr_hands` (SPEC §5.3b): a real Valve Index action set, or a deterministic scripted pair with no runtime at all — the whole hands/laser/grab lane runs headless, 4-up on the desktop, this way |
| `FARFALL_VR_SCRIPT=idle\|reach-stick\|grab-stick-roll\|throttle-push\|laser-menu` | `idle` | The synthetic hands' own script (a pure function of elapsed bench time, one log line per state transition — `"VR hands: right GRAB stick t=2.1s"`): `idle` both hands on the lap; `reach-stick` the right hand travels to the stick and holds; `grab-stick-roll` reaches, grabs, then rolls it back and forth; `throttle-push` the left hand grabs the throttle and pushes 0→80%; `laser-menu` the right hand's aim ray sweeps the open menu and presses. Shared with fable/vr's synthetic head (`still\|look\|lean\|nod\|spin`) — a name from one side is simply idle/still on the other |
