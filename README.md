...
**Holo3PP**: press N and a hologram panel opens on the glass with a live, perfect third-person projection of your own ship — the chase view (the same fighter SDF, Sun-lit, nozzles glowing with the engines; shield ripples and the warp-stop after-image stay glued to the hull) rendered to a small texture every frame and projected in the cockpit. Third person without ever leaving first person. Drag it anywhere, size it with HOLO SIZE (GFX page). Y swaps the whole screen to the raw chase camera (CAMERA on the GFX page) — the dev/bench reference the holo is measured against.
