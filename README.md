...
**Controls** — WASD translate, R/F up/down, arrows pitch/yaw, Q/E roll — or a **HOTAS**: a Thrustmaster T.Flight HOTAS 4 flies out of the box (stick pitch/roll, twist yaw, lever throttle, rocker strafe, trigger fires; every button a named control), any other stick after Esc → STICK → SETUP WIZARD, which walks the stick control by control and learns which axis and which way (`docs/HOTAS.md`).
